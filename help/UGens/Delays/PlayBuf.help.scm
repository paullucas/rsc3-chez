...
;; must be a fixed integer. The architechture of the SynthDef cannot
;; change after it is compiled.  Warning: if you supply a bufnum of a
;; buffer that has a different numChannels then you have specified to
;; the PlayBuf, it will fail silently.

;; bufnum - the index of the buffer to use

;; rate - 1.0 is the server's sample rate, 2.0 is one octave up, 0.5
;; is one octave down -1.0 is backwards normal rate etc. Interpolation
;; is cubic.  Note: If the buffer's sample rate is different from the
;; server's, you will need to multiply the desired playback rate by
;; (file's rate / server's rate). The UGen BufRateScale.kr(bufnum)
;; returns this factor. See examples below. BufRateScale should be
;; used in virtually every case.
