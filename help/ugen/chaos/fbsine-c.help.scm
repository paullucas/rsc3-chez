;; (fbsine-c rate freq im fb a c xi yi)
...
(audition
 (out 0 (mul (fbsine-c ar (fdiv sample-rate 4) 1 0.1 1.1 0.5 0.1 0.1) 
	     0.2)))
...
(let ((fb (line kr 0.01 4 10 do-nothing)))
  (audition
   (out 0 (mul (fbsine-c ar sample-rate 1 fb 1.1 0.5 0.1 0.1) 0.2))))

;; increase phase multiplier

(let ((a (line kr 1 2 10 do-nothing)))
  (audition
   (out 0 (mul (fbsine-c ar sample-rate 1 0 a 0.5 0.1 0.1) 0.2))))

;; randomly modulate parameters

(let* ((x (mouse-x kr 1 12 0 0.1))
       (f (lambda (m a) (mul-add (lfnoise2 kr x) m a))))
  (audition
   (out 0 (mul (fbsine-c ar
			 (f 1e4 1e4)
			 (f 32 33)
			 (f 0.5 0)
			 (f 0.05 1.05)
